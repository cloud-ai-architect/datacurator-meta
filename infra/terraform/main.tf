...
# window.DATACURATOR_API_URL and fell back to "https://api.example.com",
...
resource "aws_s3_object" "ui_config" {
  ...
  content = <<-JS
    ...
    window.DATACURATOR_API_URL = "${module.apigateway.api_url}";
    window.DATACURATOR_ENV = "${var.environment}";
  JS
